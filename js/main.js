...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (modal && modalImg) {
   ...
   // ADDED .testimonial-img here
   document.querySelectorAll('.imgwrap img, .polaroid-img, .stack-front, .cat-img-wrap img, #prod-img, .testimonial-img').forEach(img => {
  ...
  // 10. PRODUCT REDIRECT LOGIC
  document.querySelectorAll('.gallery .product .imgwrap img').forEach(img => {
    img.style.cursor = 'pointer';
    img.addEventListener('click', (e) => {
      e.preventDefault();
      const productEl = img.closest('.product');
      if (productEl && productEl.dataset.id) {
        window.location.href = `product.html?id=${productEl.dataset.id}`;
      }
    });
  });
  ...
  if (searchInput && searchResults && typeof productsData !== 'undefined') {
    searchInput.addEventListener('input', (e) => {
      ...
      const filtered = productsData.filter(p => 
        ...
        p.customizations.some(c => c.toLowerCase().includes(query))
      );
        ...
        filtered.forEach(p => {
          ...
          a.innerHTML = `
            ...
            <div class="search-result-info">
              ...
              <span class="search-result-sku">SKU: ${p.id}</span>
            </div>
          `;
